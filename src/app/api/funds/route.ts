...
export async function GET(request: Request) {
  try {
    ...
    let sql = 'SELECT * FROM schema_beta.funds';
...
export async function POST(request: Request) {
  try {
    ...
    const result = await db.query("SELECT fund_id FROM schema_beta.funds ORDER BY fund_id DESC LIMIT 1");
    let nextId = 1;
    ...
    try {
      await db.query(
        'INSERT INTO schema_beta.funds (fund_id, fund_abbr, fund_name_th, fund_name_en, fiscal_year, source_agency, start_period, end_period, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
...
export async function PUT(request: Request) {
  try {
    ...
    await db.query(
      `DELETE FROM schema_beta.embedding WHERE metadata->'Metadata'->>'source_id' = $1`,
      [fund_id]
    );
    // Update fund
    await db.query(
      'UPDATE schema_beta.funds SET fund_abbr=$2, fund_name_th=$3, fund_name_en=$4, fiscal_year=$5, source_agency=$6, start_period=$7, end_period=$8, status=$9 WHERE fund_id=$1',
      [fund_id, fund_abbr, fund_name_th, fund_name_en, fiscal_year, source_agency, startPeriod, endPeriod, status]
    );
    // Delete embeddings for related manual steps (if any)
    const manualResult = await db.query('SELECT chunk_id FROM schema_beta.manual WHERE fund_abbr LIKE $1', [`%${fund_abbr}%`]);
    for (const row of manualResult.rows) {
      await db.query(
        `DELETE FROM schema_beta.embedding WHERE metadata->>'source_table' = $1 AND metadata->>'source_id' = $2`,
        ['manual', row.chunk_id]
      );
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    const fundResult = await db.query('SELECT fund_abbr FROM schema_beta.funds WHERE fund_id=$1', [fund_id]);
    ...
    const manualResult = await db.query(
      `SELECT chunk_id, document_title, step_number, topic, fund_abbr FROM schema_beta.manual WHERE fund_abbr LIKE $1`,
      [`%${fund_abbr}%`]
    );
    ...
    if (force && affectedSteps.length > 0) {
      ...
      for (const step of affectedSteps) {
        ...
        if (abbrs.length === 0) {
          ...
          await db.query('DELETE FROM schema_beta.manual WHERE chunk_id = $1', [step.chunk_id]);
        } else {
          // ถ้ายังเหลือกองทุนอื่น ให้อัปเดตและ Re-embed (n8n จะสร้าง embedding ใหม่ให้เอง)
          const newAbbr = abbrs.join(' ').replace(/\s+/g, ' ').trim();
          await db.query('UPDATE schema_beta.manual SET fund_abbr = $1 WHERE chunk_id = $2', [newAbbr, step.chunk_id]);
          // --- Trigger n8n webhook for re-embedding ---
          try {
            const manualRes = await db.query('SELECT * FROM schema_beta.manual WHERE chunk_id = $1', [step.chunk_id]);
    ...
    // Delete fund
    await db.query('DELETE FROM schema_beta.funds WHERE fund_id=$1', [fund_id]);
    return NextResponse.json({ success: true });
  } catch (error) {
