  ...
  try {
    ...
    if (search) {
      terms = await db.query(
        `SELECT * FROM ${schema}.term WHERE word ILIKE $1 ORDER BY word_id asc`,
        [`%${search}%`]
      );
      console.log("DB Result:", terms.rows.length, "rows found");
    } else {
      terms = await db.query(`SELECT * FROM ${schema}.term ORDER BY word_id asc`);
      console.log("DB Result:", terms.rows.length, "rows found");
    }
    // ดึง word_type ไม่ซ้ำทั้งหมด
    const typeResult = await db.query(`SELECT DISTINCT word_type FROM ${schema}.term WHERE word_type IS NOT NULL AND word_type <> '' ORDER BY word_type ASC`);
...
export async function POST(request: Request) {
  try {
    ...
    const schema = process.env.DB_SCHEMA || 'schema_beta';
    const max = await db.query(`SELECT MAX(CAST(SUBSTRING(word_id, 3) AS INTEGER)) AS maxnum FROM ${schema}.term WHERE word_id LIKE 'W-%'`);
    let nextNum = 1;
    if (max.rows[0].maxnum) nextNum = max.rows[0].maxnum + 1;
    const word_id = `W-${String(nextNum).padStart(3, '0')}`;
    const result = await db.query(
      `INSERT INTO ${schema}.term (word_id, word, meaning, word_type) VALUES ($1, $2, $3, $4) RETURNING *`,
...
export async function PUT(request: Request) {
  try {
    ...
    const schema = process.env.DB_SCHEMA || 'schema_beta';
    await db.query(
      `DELETE FROM ${schema}.embedding WHERE metadata->'Metadata'->>'word_id' = $1`,
      [word_id]
    );
    const result = await db.query(
      `UPDATE ${schema}.term SET word = $2, meaning = $3, word_type = $4 WHERE word_id = $1 RETURNING *`,
      [word_id, word, meaning, word_type]
    );
...
export async function DELETE(request: Request) {
  try {
    ...
    await db.query(`DELETE FROM ${schema}.term WHERE word_id = $1`, [word_id]);
